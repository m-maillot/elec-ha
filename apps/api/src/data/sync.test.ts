...
describe('synchronisation de la consommation', () => {
  ...
  it('charge par tranches, calcule kwh depuis change, et ne redemande que les 7 derniers jours', async () => {
    ...
    const r2 = await syncConsumption({ db: app.ctx.db, clock, ha: client, statisticId: 'sensor.energy', from: '2026-01-01', to: '2026-02-15', today: '2026-03-01' });
    expect(r2).toEqual({ chunks: 0, hoursStored: 0, daysRequested: 0 });
    const r3 = await syncConsumption({ db: app.ctx.db, clock, ha: client, statisticId: 'sensor.energy', from: '2026-01-01', to: '2026-02-15', today: '2026-02-16' });
  ...
  it('retombe sur la différence des sum quand change est absent', async () => {
    ...
    await configure(app, ha);
    const client = new HaClient(ha.url, ha.token);
    const r = await syncConsumption({ db: app.ctx.db, clock, ha: client, statisticId: 'sensor.energy', from: '2026-01-10', to: '2026-01-10', today: '2026-03-01' });
    ...
    const r2 = await syncConsumption({ db: app.ctx.db, clock, ha: client, statisticId: 'sensor.energy', from: '2026-01-11', to: '2026-01-11', today: '2026-03-01' });
  ...
  it('considère les jours incomplets comme à recharger', async () => {
    ...
    await configure(app, ha);
    await syncConsumption({ db: app.ctx.db, clock, ha: new HaClient(ha.url, ha.token), statisticId: 'sensor.energy', from: '2026-01-10', to: '2026-01-11', today: '2026-03-01' });
    expect(selectDaysToFetch(app.ctx.db, clock, '2026-01-10', '2026-01-11', '2026-03-01')).toEqual(['2026-01-10', '2026-01-11']);
  });
